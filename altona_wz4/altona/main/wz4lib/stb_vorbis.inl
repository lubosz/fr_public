// Ogg Vorbis I audio decoder  -- version 0.998
//
...
#define STB_VORBIS_FAST_HUFFMAN_LENGTH   8
#endif
...
#endif
   uint32 *codewords;
   int    fast_huffman[FAST_HUFFMAN_TABLE_SIZE];
...
static int start_decoder(vorb *f)
{
      ...
      if (c->lookup_type > 0) {
         ...
         if (c->lookup_type == 1) {
            ...
            if (sparse) {
               ...
               c->multiplicands = (uint16 *) setup_malloc(f, sizeof(c->multiplicands[0]) * c->sorted_entries * c->dimensions);
            } else
               c->multiplicands = (uint16 *) setup_malloc(f, sizeof(c->multiplicands[0]) * c->entries        * c->dimensions);
   ...
   // 1.
   {
      uint32 imdct_mem = f->blocksize_1 * sizeof(float);
      uint32 classify_mem;
